...
def get_ops():
    ...
    data = req.json()
    cfg.OPS.clear()
    for m in data["chatters"]["moderators"]:
        cfg.OPS[m] = "mod"
    for g in data["chatters"]["global_mods"]:
        cfg.OPS[g] = "glb"
    for a in data["chatters"]["admins"]:
        cfg.OPS[a] = "adm"
    for s in data["chatters"]["staff"]:
        cfg.OPS[s] = "stf"
...
def main():
    ...
    threading.Thread(get_ops()).start()
    ...
    while True:
        ...
        for line in temp:
            ...
            if line.find("PRIVMSG") != -1:
                ...
                if is_command(msg):
                    if is_mod(usr):
                        parse_command([x.lower() for x in msg[1:].split(" ")])
                    else:
                        chat(
                            s, "@{} You do not have the permissions to justify this command.".format(usr))
